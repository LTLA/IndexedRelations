...
Use the `partnerFeatures()` and `partners()` methods to extract the partners:
...
partners(rel)[,2] <- sample(length(ir), length(rel), replace=TRUE)
...
Note that all of these methods are agnostic to the identities of the underlying feature sets, as long as the feature classes are comparable across objects.
